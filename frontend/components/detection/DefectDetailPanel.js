  ...
  if (!sorted.length) {
    return (
      <div style={{ textAlign: 'center', padding: '32px 0' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: '0.75rem', marginTop: 4 }}>PCB passed quality check</p>
      </div>
    );
  }
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: '0.82rem', fontWeight: 600, color: 'rgba(255,255,255,0.9)' }}>{defect.type}</span>
              </div>
            ...
            <div style={{ display: 'flex', gap: 12, paddingLeft: 16 }}>
              <div>
                <span style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.3)' }}>Confidence: </span>
                <span style={{ fontSize: '0.7rem', color: color, fontWeight: 500 }}>{defect.confidence?.toFixed(1)}%</span>
              </div>
              {defect.location && (
                <div>
                  <span style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.3)' }}>Location: </span>
                  <span style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.6)' }}>{defect.location}</span>
                </div>
              )}
            </div>
            ...
            <AnimatePresence>
              {isSelected && defect.description && (
                <motion.p initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                  style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.4)', marginTop: 8, paddingLeft: 16, lineHeight: 1.5 }}>
                  {defect.description}
                </motion.p>
              )}
            </AnimatePresence>
