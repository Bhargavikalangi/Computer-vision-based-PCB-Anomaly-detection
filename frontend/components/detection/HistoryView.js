  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      ...
      <GlassCard delay={0.1} style={{ marginBottom: 20 }}>
        <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '0.95rem', fontWeight: 600, marginBottom: 16, color: 'var(--text-primary)' }}>This Week</h3>
        <ResponsiveContainer width="100%" height={180}>
          <BarChart data={weekData} barGap={4}>
            <CartesianGrid strokeDasharray="3 3" stroke="var(--border-glass)" vertical={false} />
            <XAxis dataKey="day" tick={{ fill: 'var(--chart-muted)', fontSize: 11 }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fill: 'var(--chart-muted)', fontSize: 11 }} axisLine={false} tickLine={false} />
            <Tooltip contentStyle={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-glass)', borderRadius: 10, color: 'var(--text-primary)', fontSize: '0.8rem' }} />
      ...
      <GlassCard delay={0.2}>
        <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '0.95rem', fontWeight: 600, marginBottom: 20, color: 'var(--text-primary)' }}>Timeline</h3>
        {loading && <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', textAlign: 'center', padding: '20px 0' }}>Loading...</p>}
        {!loading && items.length === 0 && (
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            <div style={{ fontSize: 36, marginBottom: 10, color: 'var(--text-muted)' }}>◷</div>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem' }}>No analyses yet</p>
          </div>
        )}
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {items.map((item, i) => (
            <motion.div key={item.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
              ...
              {i < items.length - 1 && (
                <div style={{ position: 'absolute', left: 11, top: 24, bottom: 0, width: 1, background: 'var(--border-glass)' }} />
              )}
              <div style={{
              ...
              <div style={{ flex: 1, background: 'var(--panel-bg)', borderRadius: 10, padding: '12px 14px', border: '1px solid var(--border-glass)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <span style={{ fontSize: '0.83rem', fontWeight: 500, color: 'var(--text-primary)' }}>{item.filename}</span>
                ...
                <div style={{ marginTop: 4, display: 'flex', gap: 12 }}>
                  ...
                  {item.processing_time && <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>{item.processing_time}s</span>}
                </div>
              </div>
            </motion.div>
          ))}
