  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: 28 }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.875rem' }}>Generate and download comprehensive analysis reports</p>
      </motion.div>

      {/* Generate new report */}
      <GlassCard delay={0.1} style={{ marginBottom: 24 }}>
        <h3 style={{ fontFamily: 'Syne, sans-serif', fontSize: '0.95rem', fontWeight: 600, marginBottom: 18 }}>Generate Report</h3>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
          {[
            ...
            <motion.button key={item.type}
              ...
              style={{
                background: generating === item.type ? 'rgba(99,179,237,0.1)' : 'rgba(255,255,255,0.03)',
                border: `1px solid ${generating === item.type ? 'rgba(99,179,237,0.3)' : 'rgba(255,255,255,0.08)'}`,
              ...
              <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '0.85rem', fontWeight: 600, color: 'rgba(255,255,255,0.8)', marginBottom: 4 }}>
                {generating === item.type ? 'Generating...' : item.label}
              </div>
              <div style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.3)' }}>PDF + CSV</div>
            </motion.button>
          ))}
        </div>
      </GlassCard>
      ...
      {/* Reports list */}
      <GlassCard delay={0.2}>
        <h3 style={{ fontFamily: 'Syne, sans-serif', fontSize: '0.95rem', fontWeight: 600, marginBottom: 16 }}>Generated Reports</h3>

        {loading && (
          <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: '0.85rem', textAlign: 'center', padding: '20px 0' }}>Loading...</p>
        )}

        {!loading && reports.length === 0 && (
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            <div style={{ fontSize: 36, marginBottom: 10 }}>◻</div>
            <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.875rem' }}>No reports generated yet</p>
            <p style={{ color: 'rgba(255,255,255,0.25)', fontSize: '0.78rem', marginTop: 4 }}>Click a button above to generate your first report</p>
          </div>
        )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {reports.map((report, i) => (
            <motion.div key={report.id}
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: '0.85rem', fontWeight: 500, color: 'rgba(255,255,255,0.85)' }}>{report.title}</div>
